...
def main():
    parser = argparse.ArgumentParser(
        description="A tiny reminder that will notify you when you need a rest"
    )
    parser.add_argument("-v", "--version", action="version", version="1.0")
    parser.add_argument(
        "-w", "--work", type=int, help="working duration(in minutes)", default=45
    )
    parser.add_argument(
        "-r", "--rest", type=int, help="rest interval(in minutes)", default=5
    )
